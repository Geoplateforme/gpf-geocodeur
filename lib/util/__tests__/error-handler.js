...
function createError(statusCode, message, properties = {}) {
...
test('errorHandler / error with status code 400 and detail property', async t => {
  const server = createServer((req, res, next) => {
    next(createError(400, 'foo', {detail: ['bar']}))
  })
  ...
  t.deepEqual(body, {code: 400, message: 'foo', detail: ['bar']})
})
