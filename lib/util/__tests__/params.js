...
import {extractParams, parseValue, isFirstCharValid, isDepartmentcodeValid, extractParam} from '../params.js'
...
test('extractParams / missing q parameter', t => {
  const error = t.throws(() => extractParams({}, {operation: 'search'}, PARAMS), {message: 'Failed parsing query'})
  t.deepEqual(error.detail, ['q is a required param'])
})

test('extractParams / missing q but parcel only', t => {
  t.deepEqual(
    extractParams({index: 'parcel'}, {operation: 'search'}, PARAMS),
    {
      indexes: ['parcel'],
      limit: 5
    }
