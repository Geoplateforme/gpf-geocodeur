...
import {normalizeQuery} from './querystring.js'
...
export function extractSingleParams(query, paramsConfig) {
  const params = {}
  const errors = []

  for (const [paramName, definition] of Object.entries(paramsConfig)) {
    try {
...
export function extractParams(query, {operation}, paramsConfig) {
  const parsedParams = extractSingleParams(normalizeQuery(query), paramsConfig)
